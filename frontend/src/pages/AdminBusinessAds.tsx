...
export default function AdminBusinessAds() {
  ...
  const handleSendRenewalReminder = async (id: string) => {
    try {
      await api.businessAds.sendRenewalReminder(id);
              ...
              <table className="w-full text-sm">
                <thead className="border-b border-slate-100 dark:border-slate-800">
                  <tr className="text-left text-xs font-medium text-slate-500">
                    ...
                    <th className="px-4 py-3 hidden md:table-cell"><Eye className="h-3.5 w-3.5 inline" /> Views / <MousePointerClick className="h-3.5 w-3.5 inline" /> Clicks</th>
                    <th className="px-4 py-3 hidden lg:table-cell">Impressions / CTR</th>
                  ...
                  {adminAds.map((ad) => (
                    <tr key={ad.id} className="hover:bg-slate-50 dark:hover:bg-slate-800/50">
                      <td className="px-4 py-3">
                        ...
                        {ad.expiresAt && <p className="text-xs text-slate-400">Exp: {ad.expiresAt.slice(0, 10)}</p>}
                      </td>
                      ...
                      <td className="px-4 py-3 hidden lg:table-cell text-xs text-slate-500">
                        {(ad.impressions ?? 0).toLocaleString()} / {(ad.ctr ?? 0).toFixed(2)}%
                      </td>
                      <td className="px-4 py-3">
                        <div className="flex flex-wrap gap-1.5">
                          ...
                          {isExpiringWithin7Days(ad.expiresAt) && (
                            <button onClick={() => void handleSendRenewalReminder(ad.id)} className="flex items-center gap-1 rounded-lg bg-amber-50 px-2 py-1 text-xs font-medium text-amber-700 hover:bg-amber-100" title="Send renewal reminder">
