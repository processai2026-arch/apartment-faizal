...
export default function VehicleRegistry() {
  ...
  const [form, setForm] = useState({ vehicleNo: '', vehicleType: '4-Wheeler' as Vehicle['vehicleType'], ownerName: '', apartmentNo: '' });

  const handleSubmit = async (e: React.FormEvent) => {
    e.preventDefault();
    if (!form.vehicleNo || !form.ownerName || !form.apartmentNo) { toast.error('Please fill all fields'); return; }
    const v: Vehicle = { id: `VH${Date.now()}`, ...form, type: form.vehicleType, entryTime: new Date().toISOString(), status: 'Inside' };
    try {
      await addVehicle(v);
      setForm({ vehicleNo: '', vehicleType: '4-Wheeler', ownerName: '', apartmentNo: '' });
      toast.success(`Vehicle ${form.vehicleNo} logged successfully`);
    } catch (error) {
      toast.error(error instanceof Error ? error.message : 'Could not log vehicle');
    }
  };
        ...
        <div className="bg-white rounded-2xl border border-slate-100 shadow-sm p-6">
          ...
          <form onSubmit={handleSubmit} className="space-y-4">
            ...
            <div>
              ...
              <select value={form.vehicleType} onChange={e => setForm(f => ({ ...f, vehicleType: e.target.value as Vehicle['vehicleType'] }))}
                className="w-full px-3 py-2.5 border border-slate-200 rounded-xl text-sm focus:outline-none focus:ring-2 focus:ring-indigo-500">
                {(['2-Wheeler', '4-Wheeler', 'Other'] as Vehicle['vehicleType'][]).map(t => <option key={t} value={t}>{t}</option>)}
              </select>
            </div>
