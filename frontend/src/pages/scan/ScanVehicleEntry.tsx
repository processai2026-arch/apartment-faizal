          ...
          <form onSubmit={handleSubmit} className="space-y-4">
            ...
            <div>
              ...
              <div className="grid grid-cols-2 gap-2">
                {(['2-Wheeler', '4-Wheeler', 'Other'] as Vehicle['vehicleType'][]).map(t => (
                  <button
                    key={t}
                    type="button"
                    onClick={() => setForm(f => ({ ...f, vehicleType: t }))}
                    className={`py-2.5 rounded-xl text-sm font-medium border transition-all ${form.vehicleType === t ? 'bg-indigo-600 text-white border-indigo-600 shadow-sm' : 'border-slate-200 text-slate-600 hover:border-indigo-300'}`}
                  >
                    {t}
                  </button>
                ))}
